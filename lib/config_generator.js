...
exports.generate = function(config) {
  ...
  if (config.tlsCertificateFile ? 
       !config.tlsCertificateKeyFile : config.tlsCertificateKeyFile) {
    throw new Error('Certificate file and key file must both be set!');
  }

  config.tlsCertificateFile = config.tlsCertificateFile || 'server.crt';
  config.tlsCertificateKeyFile = config.tlsCertificateKeyFile || 'server.key';
