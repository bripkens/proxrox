...
exports.start = function(config) {
  ...
  // Load the extra site (if any)
  if (config.extraSite) {
    config.extraSite = fs.readFileSync(config.extraSite, { encoding: 'utf8' });
  }
  ...
  mkdirp.sync(logDir, { mode: '755' });

  // these directories need to exist as well.
  mkdirp.sync(path.join(tmpDir, 'clientBody'), { mode: '755' });
  mkdirp.sync(path.join(tmpDir, 'proxy'), { mode: '755' });
